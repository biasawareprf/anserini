...
public class Rm3Reranker implements Reranker {
  ...
  public Rm3Reranker(Analyzer analyzer, String field, int fbTerms, int fbDocs, float originalQueryWeight,
      boolean outputQuery) {
  ...
  public ScoredDocuments rerank(ScoredDocuments docs, RerankerContext context) {
    ...
    try {
      ...
      if (context.getSearchArgs().arbitraryScoreTieBreak) {
        ...
        rs = searcher.search(finalQuery, 1000, BREAK_SCORE_TIES_BY_DOCID, true);
      }
    } catch (IOException e) {
      e.printStackTrace();
      return docs;
